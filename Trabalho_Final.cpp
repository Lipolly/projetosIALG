...
//Alguns modulos passaram por alterações para ajustes de uma etapa do trabalho para a outra
//No modulo de cadastro foi retirado um cout que mostrava o estilo da banda, foi colocado para testes devido a um erro e infelizmente esq	ueci de retirar.
//O modulo de pesquisa foi alterado para que quando ele não encontra-se o registro ele informa-se, durante ajustes na segunda etapa ouve esse erro e não foi visto na hora.
//Na segunda parte haviamos entendido que a opção de excluir um registro fazia parte de altera-lo conforme a necessidade, nesta etapa foi aperfeiçoado para mostrar mais de uma opção para buscar o registro que deseja excluir.
//Quase todos os modulos passaram por alterações puramente esteticas na parte da escrita.
